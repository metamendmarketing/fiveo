...
import { VehicleMake, VehicleModel, VehicleYear, VehicleEngine } from "@/app/lib-v2/types";
import { 
  CarFront, 
  Motorbike, 
  Anchor, 
...
export function StepVehicle({ profile, onUpdate, onNext, showTypeSelector }: Props) {
  ...
  const fetchVehicleData = useCallback(async (params: string) => {
    try {
      const response = await fetch(`/fiveo/demo/api/v2/oracle/vehicles?${params}&vehicleType=${profile.vehicleType || 'car'}`);
