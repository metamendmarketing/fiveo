      ...
      <div className="w-full max-w-2xl mx-auto">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm shadow-xl">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#00AEEF] mb-3 block">
            ...
            <input
              ...
              placeholder="e.g. 550"
              className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-5 text-white text-lg font-mono font-bold outline-none focus:border-[#00AEEF] transition-all focus:bg-white/10"
            />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm shadow-xl">
              ...
              <select
                ...
                className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-5 text-white text-sm font-bold outline-none focus:border-[#00AEEF] transition-all cursor-pointer focus:bg-white/10"
              >
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-5 backdrop-blur-sm shadow-xl">
            <label className="text-[10px] font-black uppercase tracking-[0.2em] text-[#00AEEF] mb-4 block text-center">
              Headroom Preference
            </label>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {([
                ...
                <button
                  ...
                  className={`relative p-4 rounded-xl border transition-all text-center group overflow-hidden flex flex-col items-center justify-center ${
                    profile.headroomPref === opt.value
                      ? "border-[#00AEEF] shadow-[0_0_15px_rgba(0,174,239,0.3)] text-white"
                      : "border-white/10 bg-white/5 text-white/50 hover:border-white/30 hover:bg-white/10"
                  }`}
                >
                  {profile.headroomPref === opt.value && (
                    <div className="absolute inset-0 bg-gradient-to-b from-[#00AEEF]/10 to-transparent" />
                  )}
                  <span className={`relative z-10 font-black uppercase text-[11px] block mb-1 ${profile.headroomPref === opt.value ? 'text-white' : 'text-white/80 group-hover:text-white'}`}>{opt.label}</span>
                  <span className={`relative z-10 text-[9px] block font-medium ${profile.headroomPref === opt.value ? 'text-[#00AEEF]' : 'text-white/40'}`}>{opt.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="mt-8">
          <button 
            onClick={onNext} 
            disabled={!profile.desiredSizeCC || !profile.fuelPressurePSI || !profile.headroomPref}
